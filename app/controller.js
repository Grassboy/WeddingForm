$(function(){
    ...
    var $correct_area = $('.correct-area');
    $correct_area.bind('click', function(){
        clearInterval(timer);
        $correct_area.find('.show').removeClass('show');
        $correct_area.find('.got_award').addClass('show');
    }).bind('dblclick', function(e){
        if(confirm('確定要重抽！？')) {
            var qid = $correct_area.attr('data-qid');
            var answer = $correct_area.attr('data-answer');
            startDraw(qid, answer);
        }
    });
    var startDraw = function(qid, answer){
        $correct_area.attr({
            'data-qid': qid,
            'data-answer': answer
        });
        $correct_area.children().remove();
        myFirebaseRef.ref('/tiny_exam/question'+qid+'/logs').once('value').then(function(r){
            var logs = r.val();
            console.log('所有作答結果', logs);
            var count = 0;
            var candidate = {};
            for(var user_id in logs) {
                var user = db.users[user_id];
                if(logs[user_id] == answer){
                    var $obj = $('<div class="answer-avatar"></div>').attr({
                        'data-name': user.name,
                        'data-relation': user.relation,
                        'data-user_id': user_id
                    }).css('background-image', 'url(https://i.imgur.com/'+user.avatar+'.png)').appendTo('.correct-area');
                    count++;
                    candidate[user.name] = candidate[user.name] || [];
                    candidate[user.name].push(user);
                }
                $correct_area.attr('data-msg', '共有 ' + count + ' 人答對');
            }
            if(count == 0) {
                alert('歪腰，這題通殺，沒人答對0rz');
                return;
            }
            //防止一名多投的機制
            var real_candidate = [];
            for(var k in candidate) {
                candidate[k].sort(function(){return Math.random() >= 0.5?1:-1});
                real_candidate.push(candidate[k][0]);
            }
            var award_user = real_candidate[parseInt(Math.random()*real_candidate.length, 10)];
            if($correct_area.find('[data-user_id="'+award_user.id+'"]').length == 0) {
                console.log('why');
            }
            $correct_area.find('[data-user_id="'+award_user.id+'"]').addClass('got_award');
            if(count > 1) {
                clearInterval(timer);
                timer = setInterval(function(){
                    if($('.secret-event').is('.in-question')) {
                        var $show = $correct_area.find('.show');
                        $show.removeClass('show');
                        if($show.length == 0 || $show.next().length == 0) {
                            $correct_area.children(':eq(0)').addClass('show');
                        } else {
